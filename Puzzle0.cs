...
public class Puzzle0 : MonoBehaviour
{
    ...
    void OnEnable(){
        ...
        if(DatabaseManager.instance.gameOverList.Contains(0)){//뿌리퍼즐 맞췄으면 
    ...
    public IEnumerator SuccessImplantation(){   //심기 성공시or다시뽑기후 심기 성공시 바로 진행되는 멘트.
        buttonOff();
        Invoke("SpritesOn",1f);
        broken.SetActive(false);
        afterfixed.SetActive(true);
    ...
    IEnumerator GiveCandyCoroutine(){
        ...
        if(Inventory.instance.SearchItem(2)){       //사탕이 있으면 성공
    ...
    public void OnActivatedReset(){                 // 선택된 템 초기화(databaseManager : OnActivated) 일단 사용안함
