...
public class Inventory : MonoBehaviour
{
    ...
    public Dialogue wrongTribute; // 잘못된 사용.
    public Dialogue checkAllTributes; // 잘못된 사용.
    ...
    {              //BookUpdate용, 실제로 가지고 있는 아이템 갯수 만큼 슬롯 0번부터 차례로 대입함. 초기화하고. 아이템 사용후 실행할 것.
    ...
    public void ShowMainItem(int slotNum)
    {                              //아이템 개개 버튼 누르면 설명 창 활성화.  사용하기 위해 ID도 받아옴 
        ...
        if (inventoryItemList[temp] != null)
        {
            ...
            Debug.Log(inventoryItemList[temp].isEE);
            if (inventoryItemList[temp].isEE) eE.SetActive(true);
            else eE.SetActive(false);
    ...
    {                               //다른 오브젝트에서 아이템 획득시 실행되는 함수. id만 받아와서 DB에 있는 이름과 이미지 불러옴.
    ...
    public bool SearchItem(int _itemID)
    {//아이템 목록에서 검색 : 찾아서 있으면 true 반환.
    ...
    {//커서 전환 후 버튼 클릭으로 아이템 사용    ex)puzzle0에서 햇빛 object 버튼에 주는 함수
    ...
    public void WearItem()
    {
        ...
        switch (mainItem.itemID)
        {
            ...
            case 17:
                ...
                // thePlayer.normalColor = new Color(1,1,0.3f,1);
                // thePlayer.normalColor = new Color(1,1,0.3f,1);
                thePlayer.shadowColor.b = 0.5f;
                thePlayer.normalColor.b = 0.5f;
                break;
        }
    }
        ...
        switch (mainItem.itemID)
        {
            ...
            case 18:
                ...
                if (GameMultiLang.instance.nowLang == "kr")
                    BookManager.instance.mazeManaul.SetActive(true);
                else
                    BookManager.instance.mazeManaul_en.SetActive(true);
                //thePlayer.runSpeed=9;
                break;
        }
    }
    //패시브 아이템 보유시 실행
    public void ActivatePassiveItem(int num, Sprite sprite)
    {
        switch (num)
        {
            case 14:
                //PrintFloatingText("생명력 +1");
                PrintFloating("", sprite);
                thePlayer.life = 1;
                break;
        }
    }
    public void PrintFloating(string text, Sprite sprite)
    {
    ...
    {
        if (thePlayer.milkRemain > 0)
        {
            thePlayer.milkRemain -= 1;
            if (thePlayer.milkRemain == 0)
            {
                thePlayer.ResetColor();
            }
        }
        if (thePlayer.shoe0Remain > 0)
        {
            thePlayer.shoe0Remain -= 1;
            if (thePlayer.shoe0Remain == 0)
            {
                thePlayer.runSpeed -= 1;
            }
        }
        if (thePlayer.shoe1Remain > 0)
        {
            thePlayer.shoe1Remain -= 1;
            if (thePlayer.shoe1Remain == 0)
            {
